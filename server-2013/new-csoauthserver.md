...
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/JJ205206(v=OCS.15)
ms:contentKeyID: 48273379
ms.date: 05/19/2016
...
Creates a new Open Authorization (OAuth) server for use by the organization. OAuth servers, also known as security token servers, issue security tokens used in server-to-server authentication and authorization. このコマンドレットは Lync Server 2013 で導入されました。
...
**Lync Server コントロール パネル:** The functions carried out by the New-CsOAuthServer cmdlet are not available in the Lync Server コントロール パネル.
