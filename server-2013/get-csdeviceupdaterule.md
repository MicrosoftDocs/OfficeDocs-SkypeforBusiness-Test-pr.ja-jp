...
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Gg398215(v=OCS.15)
ms:contentKeyID: 48271342
ms.date: 05/19/2016
...
Returns information about the device update rules configured for use in your organization. Device update rules are used to associate firmware updates with devices that run Lync Server. このコマンドレットは Lync Server 2010 で導入されました。
...
Device update rules can be imported (and applied to) the Web サービス service. The **Get-CsDeviceUpdateRule** cmdlet enables you to return information about the device update rules that have been imported for use in your organization.
...
<td><p>Retrieves the device update rule data from the local replica of the 中央管理ストア rather than from the 中央管理ストア itself.</p></td>
</tr>
</tbody>
</table>
