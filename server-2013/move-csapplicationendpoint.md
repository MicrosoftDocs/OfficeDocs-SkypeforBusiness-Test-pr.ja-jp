...
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Gg398188(v=OCS.15)
ms:contentKeyID: 48271285
ms.date: 05/19/2016
...
Moves an endpoint to a different Registrar pool. このコマンドレットは Lync Server 2010 で導入されました。
...
This cmdlet moves an existing endpoint contact object in Active Directory ドメイン サービス from a Microsoft Office Communications Server 2007 R2 application pool to a Lync Server application pool, or from one Lync Server application pool to another. The application associated with the given endpoint must exist on the target pool. To determine which application is associated with the endpoint, run the **Get-CsTrustedApplicationEndpoint** cmdlet or the **Get-CsDialInConferencingAccessNumber** cmdlet.
