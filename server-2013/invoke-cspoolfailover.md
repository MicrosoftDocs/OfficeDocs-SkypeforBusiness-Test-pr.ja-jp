...
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/JJ205189(v=OCS.15)
ms:contentKeyID: 48273329
ms.date: 05/19/2016
...
Invokes the failover process for a Lync Server 2013 pool. Failover refers to the process that occurs when a pool fails and the current users of that pool are then signed on to a backup pool. このコマンドレットは Lync Server 2013 で導入されました。
...
**Lync Server コントロール パネル:** The functions carried out by the **Invoke-CsPoolFailOver** cmdlet are not available in the Lync Server コントロール パネル.
