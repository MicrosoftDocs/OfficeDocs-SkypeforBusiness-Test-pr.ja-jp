...
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Gg398246(v=OCS.15)
ms:contentKeyID: 48271420
ms.date: 05/19/2016
...
Creates a new Response Group workflow. Workflows determine the actions that are taken when the 応答グループ アプリケーション receives a phone call. このコマンドレットは Lync Server 2010 で導入されました。
...
<p>For example, suppose your operating system is running under the Faroese language. This language is supported by the Windows operating system, but not by the 応答グループ アプリケーション. Therefore, you must include the Language parameter and a supported language when creating a new workflow.</p>
<p>This is required because, if no language is specified, the workflow uses the operating system language. However, that language can be used in a workflow only if it is a language supported by the 応答グループ アプリケーション.</p>
